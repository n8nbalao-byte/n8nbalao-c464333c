...
if (isset($_GET['code'])) {
    ...
    if ($httpCode !== 200) {
        ...
        header('Location: https://www.n8nbalao.com/cliente?error=token_exchange_failed');
        exit();
    }
    ...
    if (!$accessToken) {
        header('Location: https://www.n8nbalao.com/cliente?error=no_access_token');
        exit();
    }
    ...
    if (!isset($userInfo['email'])) {
        header('Location: https://www.n8nbalao.com/cliente?error=no_email');
        exit();
    }
    ...
    if ($existingCustomer) {
        // Update existing customer with google_id if not set
    ...
    // For simplicity, we'll pass customer data via URL params
    
    // Redirect to frontend with customer data
